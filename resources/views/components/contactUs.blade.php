...
<section class="relative z-10 px-5 overflow-hidden bg-white py-20 lg:py-[120px]">

  @if(session('success'))
    <div id="success-message" class="relative top-5 left-2/4 w-1/4 -translate-x-2/4 p-5 bg-green-700 text-white">
        {{ session('success') }}
    </div>
  @endif
          ...
          <div class="mb-12 max-w-[570px] lg:mb-0">
            ...
            <div class="mb-8 flex w-full max-w-[370px]">
              ...
              <div class="w-full">
                ...
                <p class="text-body-color text-base">(+389)70 707 707</p>
              </div>
            </div>
            <div class="mb-8 flex w-full max-w-[370px]">
              <div
              ...
              <div class="w-full">
                ...
                <p class="text-body-color text-base">info@lvshop.com</p>
              </div>
            </div>
          </div>
        </div>
        <div class="w-full px-4 lg:w-1/2 xl:w-5/12  bg-gray-50">
          <div class="relative rounded-lg bg-white p-8 shadow-lg sm:p-12">
            <form method="POST" action="/contact">
              @csrf
              <div class="mb-6">
                <input type="text" placeholder="Your Name" name="name" value="{{old('name')}}" class="text-body-color border-[f0f0f0] focus:border-primary w-full rounded border py-3 px-[14px] text-base outline-none focus-visible:shadow-none"/>
                @error('name')
                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror
              </div>
              <div class="mb-6">
                <input type="email" placeholder="Your Email" name="email" value="{{old('email')}}" class="text-body-color border-[f0f0f0] focus:border-primary w-full rounded border py-3 px-[14px] text-base outline-none focus-visible:shadow-none"/>
                @error('email')
                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror
              </div>
              <div class="mb-6">
                <input type="text" placeholder="Your Phone" name="number" value="{{old('number')}}"  class="text-body-color border-[f0f0f0] focus:border-primary w-full rounded border py-3 px-[14px] text-base outline-none focus-visible:shadow-none"/>
                @error('number')
                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror
              </div>
              <div class="mb-6">
                <textarea rows="6" placeholder="Your Message" name="message" value="{{old('message')}}" class="text-body-color border-[f0f0f0] focus:border-primary w-full resize-none rounded border py-3 px-[14px] text-base outline-none focus-visible:shadow-none"></textarea>
                @error('message')
                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror
              </div>
              <div>
                <button type="submit" class=" bg-blue-400 border-primary w-full rounded border p-3 text-white transition hover:bg-opacity-80" >Send Message</button>
              </div>
            </form>
          </div>
        </div>
